...
<div class="panelEdicion">
	<div class="container">
		<div class="divColumn">
			<h3>Resultados de la búsqueda del {{ $fechaFrom }} al {{ $fechaTo }}</h3>
			<br>
				...
				<ul class="ulListaNoticias">
					...
					<li class="elementLista">
						<div class="divElementNoti">
							<a class="txtTituloNV"  href="noticia/{{ $noti->idNoticia }}">{{ utf8_decode($noti->titulo) }}<a/>
								<br>
								<span>Reportero: </span>
								<a class="txtAutorNV" href="perfil/{{ $noti->idUsuario }}">{{ $noti->autor }}</a>
